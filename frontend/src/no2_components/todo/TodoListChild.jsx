...
const TodoListChild = ({ item }) => {
    ...
    return (
        <Tr $checked={item.checked}>
            <Td style={{ color: '#cbd5e1', fontWeight: 600 }}>#{item.id}</Td>
            <Td>
                <SubjectCell>
                    <CheckBox onClick={handleToggle}>
                        {item.checked
                            ? <MdCheckBox size={18} color="#6366f1" />
            ...
            <Td style={{ color: '#94a3b8', fontSize: '13px' }}>{createdAt}</Td>
            <Td>
...
const Tr = styled.tr`
    background: ${({ $checked }) => $checked ? '#f8fafc' : 'white'};
...
const SubjectText = styled.span`
    font-size: 14px;
    color: ${({ $checked }) => $checked ? '#cbd5e1' : '#0f172a'};
...
const Badge = styled.span`
    ...
    font-size: 12px;
    font-weight: 600;
    background: ${({ $checked }) => $checked ? '#f0fdf4' : '#eef2ff'};
    color: ${({ $checked }) => $checked ? '#16a34a' : '#6366f1'};
    border: 1px solid ${({ $checked }) => $checked ? '#bbf7d0' : '#c7d2fe'};
`
